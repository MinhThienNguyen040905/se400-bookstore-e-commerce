// src/components/admin/tabs/PublishersTab.tsx
import { useState, useEffect } from 'react';
import { usePublishers, usePublisherMutations } from '@/hooks/useAdminPublishers';
import type { Publisher } from '@/hooks/useAdminPublishers';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import {
    Search, Plus, Edit, Trash2, ChevronLeft, ChevronRight,
    Building2, X, Save, AlertTriangle, Loader2
} from 'lucide-react';
...
export function PublishersTab() {
    ...
    // Hooks
    const { data, isLoading, isError } = usePublishers(page, itemsPerPage);
    const { add, update, remove } = usePublisherMutations();
    ...
    // Destructure Data
    const { publishers = [], pagination } = data || {};
    ...
    const handleDelete = async () => {
        if (publisherToDelete) {
            await remove.mutateAsync(publisherToDelete);
            setPublisherToDelete(null);
        }
    };

    if (isLoading) return <div className="p-12 text-center text-stone-500">Loading publishers...</div>;
    if (isError) return <div className="p-12 text-center text-red-500">Failed to load publishers.</div>;
    ...
    return (
        <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h2 className="text-2xl font-bold font-display text-stone-900">Publisher Management</h2>
                    <p className="text-stone-500 text-sm mt-1">Total: {pagination?.totalItems || 0} publishers</p>
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead className="bg-stone-50 border-b border-stone-200">
                            <tr>
                                ...
                                <th className="px-6 py-4 text-xs font-bold text-stone-500 uppercase">Publisher Name</th>
                        ...
                        <tbody className="divide-y divide-stone-100">
                            {filteredPublishers.length > 0 ? (
                                filteredPublishers.map((pub) => (
                                    <tr key={pub.publisher_id} className="hover:bg-stone-50 transition-colors group">
                                        <td className="px-6 py-4 text-sm text-stone-500 font-mono">#{pub.publisher_id}</td>
                                        <td className="px-6 py-4 text-sm font-bold text-stone-900">
                                            <div className="flex items-center gap-2">
                                                <Building2 className="w-4 h-4 text-[#009b8f] opacity-50" />
                                                {pub.name}
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex justify-end gap-2">
                                                ...
                                                <Button
                                                    variant="ghost" size="icon"
                                                    onClick={() => setPublisherToDelete(pub.publisher_id)}
                                ...
                                <tr>
                                    <td colSpan={3} className="px-6 py-12 text-center text-stone-500">
                                        No publishers found on this page.
                                    </td>
                                </tr>
                            )}
                        </tbody>
            ...
            <PublisherModal
                ...
                onSave={async (name) => {
                    if (editingPublisher) {
                        await update.mutateAsync({ id: editingPublisher.publisher_id, name });
                    } else {
                ...
                <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in">
                    <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full animate-in zoom-in-95">
                        <div className="flex items-center gap-3 text-red-600 mb-4">
                            ...
                            <h3 className="text-lg font-bold">Delete Publisher?</h3>
                        </div>
                        <p className="text-stone-600 mb-6 text-sm leading-relaxed">
                            Are you sure? You <strong>cannot delete</strong> a publisher if they have books in the system.
                        </p>
                        <div className="flex justify-end gap-3">
                            <Button variant="outline" onClick={() => setPublisherToDelete(null)}>Cancel</Button>
...
function PublisherModal({ isOpen, onClose, publisher, onSave, isSaving }: PublisherModalProps) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
                ...
                <div className="p-6">
                    <form id="pub-form" onSubmit={handleSubmit} className="space-y-4">
                        <div className="space-y-2">
                            <Label htmlFor="name">Publisher Name <span className="text-red-500">*</span></Label>
                            <Input
                                id="name"
                                value={name}
                ...
                <div className="p-6 border-t border-stone-100 bg-stone-50 flex justify-end gap-3">
                    ...
                    <Button type="submit" form="pub-form" className="bg-[#0df2d7] text-stone-900 font-bold hover:bg-[#00dcc3]" disabled={isSaving || !name.trim()}>
