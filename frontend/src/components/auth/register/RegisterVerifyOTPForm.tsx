...
export default function RegisterVerifyOTPForm({ email, onSuccess }: { email: string; onSuccess?: () => void }) {
    const { verify } = useOTP();
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <div className="space-y-2">
                <Label htmlFor="otp" className="text-stone-600 font-medium">Enter OTP</Label>
                <Input
                    id="otp"
                    type="text"
                    placeholder="123456"
                    maxLength={6}
                    {...register('otp')}
                    className="h-12 border-stone-300 focus:border-[#0df2d7] focus:ring-[#0df2d7] text-center text-lg tracking-widest rounded-lg"
                />
                {errors.otp && <p className="text-sm text-red-500">{errors.otp.message}</p>}
            </div>
            <Button type="submit" className="w-full h-12 bg-[#0df2d7] hover:bg-[#00dcc3] text-stone-900 font-bold text-base rounded-lg" disabled={verify.isPending}>
