        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            <div className="space-y-2">
                ...
                <Input
                    ...
                    {...register('email')}
                    className="h-12 border-stone-300 focus:border-[#0df2d7] focus:ring-[#0df2d7] rounded-lg text-base"
                />
                {errors.email && <p className="text-sm text-red-500">{errors.email.message}</p>}
            </div>
            <Button type="submit" className="w-full h-12 bg-[#0df2d7] hover:bg-[#00dcc3] text-stone-900 font-bold text-base rounded-lg" disabled={request.isPending}>
                {request.isPending ? 'Sending...' : 'Start Registration'}
            </Button>
        </form>
    );
}